...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    // Tandai bahwa halaman sudah di-scroll
    if (!hasScrolled && latest > 0) {
      setHasScrolled(true);
    }

    // Logika untuk resize hanya jika sudah di-scroll
    if (hasScrolled && latest > 100) {
      setIsResized(true);
    } else {
      setIsResized(false);
    }

    // Logika untuk hide/show
    const isScrollUp = latest < lastScrollY;
    if (latest > 300) {  // Threshold untuk hide/show
      if (isScrollUp) {
        setVisible(true);
      } else {
        setVisible(false);
    ...
    >
      {React.Children.map(children, (child) =>
        React.isValidElement(child)
          ? React.cloneElement(
              child as React.ReactElement<{ visible?: boolean; isResized?: boolean }>,
              { visible, isResized }
            )
          : child
      )}
