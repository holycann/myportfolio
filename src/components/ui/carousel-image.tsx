  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        ...
        >
          <Image
            className="absolute inset-0 w-full h-full object-cover object-center opacity-100 transition-opacity duration-600 ease-in-out hover:scale-105 hover:z-50"
            style={{
              opacity: current === index ? 1 : 0.5,
              transformOrigin: 'center',
              transition: 'transform 0.3s ease-in-out, opacity 0.6s ease-in-out, z-index 0.3s',
            }}
            alt={title}
            src={src === "" ? createPlaceHolder(title || "Default Image") : src}
            onLoad={imageLoaded}
            loading="eager"
            decoding="sync"
            fill
        ...
        >
          <h2 className="text-lg md:text-2xl lg:text-3xl text-[var(--color-text-primary)]">
            {title}
          </h2>
        </article>
      </li>
...
const CarouselControl = ({
  ...
  return (
    <button
      className={`w-10 h-10 flex items-center mx-2 justify-center bg-[var(--color-bg-secondary)] border-3 border-transparent rounded-full focus:border-[var(--color-secondary)] focus:outline-none hover:-translate-y-0.5 active:translate-y-0.5 transition duration-200 ${
